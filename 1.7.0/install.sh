...
if type "curl" >/dev/null 2>&1; then
	curl -L --progress-bar "https://github.com/dunglas/frankenphp/releases/v1.7.0/download/${THE_ARCH_BIN}" -o "${DEST}"
elif type "wget" >/dev/null 2>&1; then
	${SUDO} wget "https://github.com/dunglas/frankenphp/releases/v1.7.0/download/${THE_ARCH_BIN}" -O "${DEST}"
else
